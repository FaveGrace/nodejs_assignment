...
app.get("/drugs/antibiotics", (req, res) => {
    const antibiotic = drugs.filter((drug) => drug.category === "Antibiotic")
    res.json(antibiotic)
})
...
app.post("/drugs/by-category", (req, res) => {
    ...
    const filteredByCategory = drugs.filter(drug => drug.category.toLowerCase() === category.toLowerCase())
...
app.get("/drugs/prescription", (req, res) => {
    const isPrescription = drugs.filter((drug) => drug.isPrescriptionOnly === true)
...
app.get("/drugs/formatted", (req, res) => {
    const mappedNewArray = drugs.map(drug => `Drug: ${drug.name} - ${drug.dosageMg}mg`)
    res.json(mappedNewArray)
})   
   
//    7.GET /drugs/low-stock   
//    Return all drugs where stock is less than 50.
app.get("/drugs/low-stock", (req, res) => {
    const filteredStock = drugs.filter((drug) => drug.stock < 50)
...
app.get("/drugs/non-prescription", (req, res) => {
    const notPrescription = drugs.filter((drug) => !drug.isPrescriptionOnly)
    res.json(notPrescription)
})
...
drugs.forEach(drug => {
    ...
    res.json({category: "Analgesic", analgesic})
})